...
export interface DataPathEnvironment {
  readonly INWSUS_DATA_PATH?: string;
  readonly APPDATA?: string;
  readonly USERPROFILE?: string;
  readonly HOME?: string;
}

/** Resolve the per-user inwsus data directory without embedding a developer profile path. */
export function resolveInwsusDataPath(
  environment: DataPathEnvironment = process.env,
  roamingAppDataFallback?: string,
): string {
  const configured = environment.INWSUS_DATA_PATH?.trim();
  ...
  return path.resolve(appData, 'inwsus');
}
