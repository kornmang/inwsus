...
describe('PowerShell tunnel launcher ownership contract', () => {
  it('parses and delegates ownership to the side-effect-free helper without local lock definitions', async () => {
    const starter = path.resolve('scripts/start-inwsus-tunnel.ps1').replace(/'/g, "''");
    const helper = path.resolve('scripts/lib/inwsus-tunnel-lock.ps1').replace(/'/g, "''");
    const result = await runPowerShell(`
      ...
      $functions=@($ast.FindAll({param($node) $node -is [Management.Automation.Language.FunctionDefinitionAst] -and $node.Name -like '*InwsusTunnelLock*'},$true)).Count
      ...
      [pscustomobject]@{ errors=$errors.Count; helperErrors=$helperErrors.Count; helperSideEffects=$helperSideEffects; functions=$functions; dotSources=$dotSources; enter=(@($commands | Where-Object {$_ -eq 'Enter-InwsusTunnelLock'}).Count); release=(@($commands | Where-Object {$_ -eq 'Release-InwsusTunnelLock'}).Count) } | ConvertTo-Json -Compress
    `);
    ...
    const starterSource = await readFile(path.resolve('scripts/start-inwsus-tunnel.ps1'), 'utf8');
    expect(starterSource).toContain('inwsus-tunnel-lock.ps1');
