...
const helperPath = path.resolve('scripts/lib/inwsus-tunnel-lock.ps1');
const starterPath = path.resolve('scripts/start-inwsus-tunnel.ps1');
...
describe('PowerShell tunnel lock helper', () => {
  it('acquires a complete record and removes every publish temporary file', async () => {
    ...
    const result = await runPowerShell(`
      . '${quote(helperPath)}'
      $claim = Enter-InwsusTunnelLock -ProfileDir '${quote(root)}' -OwnerPid 7 -OwnerStartedAt '2026-08-20T00:00:00.000Z' -ProcessStartProvider { param($id) [pscustomobject]@{state='gone'} }
      $claim | ConvertTo-Json -Compress
    `);

    expect(JSON.parse(result.stdout)).toMatchObject({ acquired: true, owner: { pid: 7 } });
    expect(JSON.parse(await readFile(path.join(root, 'inwsus.tunnel.lock'), 'utf8'))).toMatchObject({ version: 1, pid: 7 });
  ...
  it.each([
    ...
    const result = await runPowerShell(`
      . '${quote(helperPath)}'
      $claim = Enter-InwsusTunnelLock -ProfileDir '${quote(root)}' -OwnerPid 8 -OwnerStartedAt '2026-08-20T00:02:00.000Z' -ProcessStartProvider { param($id) ${provider} }
      $claim | ConvertTo-Json -Compress
    `);
  ...
  it('leaves the lock intact for a wrong owner and releases it for the exact owner', async () => {
    ...
    const result = await runPowerShell(`
      ...
      $wrongReleased = Release-InwsusTunnelLock -ProfileDir '${quote(root)}' -Owner $wrong
      $stillThere = Test-Path -LiteralPath (Join-Path '${quote(root)}' 'inwsus.tunnel.lock')
      $rightReleased = Release-InwsusTunnelLock -ProfileDir '${quote(root)}' -Owner $claim.owner
      [pscustomobject]@{ wrongReleased=$wrongReleased; stillThere=$stillThere; rightReleased=$rightReleased; existsAfter=(Test-Path -LiteralPath (Join-Path '${quote(root)}' 'inwsus.tunnel.lock')) } | ConvertTo-Json -Compress
    `);
  ...
  it.each([
    ...
    const result = await runPowerShell(`
      . '${quote(helperPath)}'
      $record = Read-InwsusTunnelLockRecord -LockPath (Join-Path '${quote(root)}' 'inwsus.tunnel.lock')
      if($null -eq $record){'INVALID'}else{'VALID'}
    `);

    expect(result.stdout).toBe('INVALID');
    expect(JSON.parse(await readFile(path.join(root, 'inwsus.tunnel.lock'), 'utf8'))).toEqual(record);
  });

  it('cleans its publish file when invalid fixed metadata rejects acquisition', async () => {
    const root = await temporaryDirectory();
    await writeFile(path.join(root, 'inwsus.tunnel.lock'), '{broken', 'utf8');

    const result = await runPowerShell(`
      . '${quote(helperPath)}'
      try { Enter-InwsusTunnelLock -ProfileDir '${quote(root)}' -OwnerPid 7 -OwnerStartedAt '2026-08-20T00:00:00.000Z' -ProcessStartProvider { param($id) [pscustomobject]@{state='gone'} }; exit 2 } catch { Write-Output $_.Exception.Message }
    `);
  ...
  it('fails closed and preserves the lock when the process probe is unverifiable', async () => {
    ...
    const result = await runPowerShell(`
      . '${quote(helperPath)}'
      try { Enter-InwsusTunnelLock -ProfileDir '${quote(root)}' -OwnerPid 88 -OwnerStartedAt '2026-08-20T00:01:00.000Z' -ProcessStartProvider { param($id) [pscustomobject]@{state='unverifiable';reason='access_denied'} }; exit 2 } catch { Write-Output $_.Exception.Message }
    `);
    expect(result.stdout).toContain('owner liveness is unverifiable: access_denied');
    expect(JSON.parse(await readFile(path.join(root, 'inwsus.tunnel.lock'), 'utf8'))).toEqual(existing);
  });
  ...
  it('keeps the newly published owner acquired when stale quarantine cleanup is obstructed', async () => {
    ...
    const result = await runPowerShell(`
      . '${quote(helperPath)}'
      $claim = Enter-InwsusTunnelLock -ProfileDir '${quote(root)}' -OwnerPid 971 -OwnerStartedAt '2026-08-20T00:01:00.000Z' -ProcessStartProvider { param($id) [pscustomobject]@{state='gone'} } -AfterStaleQuarantine {
        $quarantine = Get-ChildItem -LiteralPath '${quote(root)}' -Filter 'inwsus.tunnel.lock.stale.*' | Select-Object -First 1
    ...
    expect(JSON.parse(await readFile(path.join(root, 'inwsus.tunnel.lock'), 'utf8'))).toMatchObject({ pid: 971 });
  });
  ...
  it('serializes a PowerShell stale reclaimer against a second reclaimer and fresh third publisher', async () => {
    ...
    const first = spawnPowerShellCapture(`
      . '${quote(helperPath)}'
      $claim = Enter-InwsusTunnelLock -ProfileDir '${quote(root)}' -OwnerPid 981 -OwnerStartedAt '2026-08-20T00:01:00.000Z' -ProcessStartProvider { param($id) [pscustomobject]@{state='gone'} } -AfterStaleQuarantine {
    ...
    const contender = (pid: number, startedAt: string): Promise<{ stdout: string; stderr: string }> => runPowerShell(`
      . '${quote(helperPath)}'
      $claim = Enter-InwsusTunnelLock -ProfileDir '${quote(root)}' -OwnerPid ${pid} -OwnerStartedAt '${startedAt}' -ProcessStartProvider { param($id) if($id -eq 981){[pscustomobject]@{state='live';processStartedAt='2026-08-20T00:01:00.000Z'}}else{[pscustomobject]@{state='gone'}} }
      $claim | ConvertTo-Json -Compress
    `);
    ...
    expect(await readFile(path.join(root, 'inwsus.tunnel.lock'), 'utf8')).toContain('"pid":981');
  });
});
...
describe('production PowerShell tunnel starter integration', () => {
  it('loads the helper, reports its real concurrent owner, and never invokes the configured client', async () => {
    ...
    await writeFile(path.join(profileDir, 'inwsus.runtime.secret'), 'not-read-while-lock-is-owned', 'utf8');
  ...
  it('does not erase the active owner stop marker when the starter loses the lock', async () => {
    ...
    const profileDir = path.join(root, 'tunnel-client');
    const releaseSignal = path.join(root, 'release');
    const marker = path.join(profileDir, 'inwsus.tunnel.stop');
    const fakeClient = path.join(root, 'fake-tunnel-client.cmd');
    await mkdir(profileDir, { recursive: true });
    await writeFile(path.join(profileDir, 'inwsus.runtime.secret'), 'not-read', 'utf8');
...
async function temporaryDirectory(): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), 'inwsus-ps-lock-'));
...
async function writeLock(root: string, record: unknown): Promise<void> {
  ...
  await writeFile(path.join(root, 'inwsus.tunnel.lock'), JSON.stringify(record), 'utf8');
}
...
async function startHolder(profileDir: string, releaseSignal: string): Promise<{ child: ChildProcess; pid: number }> {
  const script = `
    . '${quote(helperPath)}'
    $started = (Get-CimInstance Win32_Process -Filter "ProcessId = $PID").CreationDate.ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ss.fffZ', [Globalization.CultureInfo]::InvariantCulture)
    $claim = Enter-InwsusTunnelLock -ProfileDir '${quote(profileDir)}' -OwnerPid $PID -OwnerStartedAt $started -ProcessStartProvider { param($id) [pscustomobject]@{state='gone'} }
    Write-Output "READY:$PID"
    [Console]::Out.Flush()
    while(-not (Test-Path -LiteralPath '${quote(releaseSignal)}')) { Start-Sleep -Milliseconds 20 }
    [void](Release-InwsusTunnelLock -ProfileDir '${quote(profileDir)}' -Owner $claim.owner)
  `;
